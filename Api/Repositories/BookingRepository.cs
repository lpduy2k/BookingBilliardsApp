        ...
        public dynamic GetList(Guid userId, int pageNumber, int pageSize)
        {
            if (pageNumber == 0 && pageSize == 0 && userId == Guid.Empty)
            {
                var bookingList = from booking in _context.Bookings
                                  ...
                                  select new { ClubId = bidaClub.Id, TimeBooking = booking.TimeBooking, TotalPrice = booking.TotalPrice, BidaTableName = bidaTable.Name, BidaClubName = bidaClub.Name, ImageTable = bidaTable.Image, AddressClub = bidaClub.Address, Status = booking.Status };
